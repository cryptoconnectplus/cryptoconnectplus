...
//CryptoConnectPlus only features
...
const char * const BITCOIN_CONF_FILENAME = "cryptoconnectplus.conf";
const char * const BITCOIN_PID_FILENAME = "cryptoconnectplusd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "cryptoconnectplus" is a composite category enabling all CryptoConnectPlus-related debug output
                if(ptrCategory->count(std::string("cryptoconnectplus"))) {
...
#else
    const char* pszModule = "cryptoconnectplus";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CryptoConnectPlus
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CryptoConnectPlus
    // Mac: ~/Library/Application Support/CryptoConnectPlus
    // Unix: ~/.cryptoconnectplus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CryptoConnectPlus";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CryptoConnectPlus";
#else
    // Unix
    return pathRet / ".cryptoconnectplus";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty cryptoconnectplus.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override cryptoconnectplus.conf
